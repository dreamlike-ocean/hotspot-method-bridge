...
final class MethodAddressResolver {
    ...
    private static final MethodHandle CHECKED_RESOLVE_JMETHOD_ID = HotSpotMemory.LINKER.downcallHandle(
    ...
    private static void register(MemorySegment env) {
        try {
            ...
            MemorySegment upcall = HotSpotMemory.LINKER.upcallStub(
    ...
    private static MethodHandle jniFunction(MemorySegment env, int index, FunctionDescriptor descriptor) {
        long functions = HotSpotMemory.getAddress(env.address());
        long function = HotSpotMemory.getAddress(functions + (long) index * ValueLayout.ADDRESS.byteSize());
        return HotSpotMemory.LINKER.downcallHandle(MemorySegment.ofAddress(function), descriptor);
    }
    ...
    private static MemorySegment currentJniEnv() {
        try {
            ...
            long functions = HotSpotMemory.getAddress(vm.address());
            long getEnv = HotSpotMemory.getAddress(functions + 6L * ValueLayout.ADDRESS.byteSize());
            MethodHandle getEnvHandle = HotSpotMemory.LINKER.downcallHandle(
