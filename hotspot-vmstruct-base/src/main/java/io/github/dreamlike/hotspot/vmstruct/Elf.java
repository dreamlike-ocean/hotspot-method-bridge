...
public final class Elf implements NativeSymbols {
    private static final int SHT_SYMTAB = 2;
    private static final int SHT_DYNSYM = 11;
    private static final CodeBlobLayout CODE_BLOB_LAYOUT = CodeBlobLayout.load();
    private static final MethodHandle CODE_CACHE_BLOB_CREATE_MH = HotSpotMemory.LINKER.downcallHandle(
    ...
    public Elf() {
        try {
            bytes = java.nio.file.Files.readAllBytes(HotSpotLibrary.libjvmPath());
            ...
            base = HotSpotLibrary.runtimeAddress("gHotSpotVMStructs") - fileSymbol("gHotSpotVMStructs");
    ...
    public NativeCode installCodeBlob(String name, byte[] code) {
        try {
            ...
            HotSpotMemory.memory(entry, code.length).copyFrom(MemorySegment.ofArray(code));
